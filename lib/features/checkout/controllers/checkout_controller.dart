...
class CheckoutController extends GetxController implements GetxService {
  ...
  // MoMo State
  final TextEditingController momoPhoneController = TextEditingController(text: '231');
  bool _hasPendingVerify = false;
  bool get hasPendingVerify => _hasPendingVerify;
  ...
  double? _pendingMaximumCodOrderAmount;
  bool _pendingFromCart = false;
  bool _pendingIsCashOnDeliveryActive = false;
  List<XFile>? _pendingOrderAttachment;
  ...
  void _startPendingTimer(int seconds) {
  ...
  Future<void> extendPendingTimer() async {
    if (!_hasPendingVerify) return;
    final prefs = await SharedPreferences.getInstance();
    final nowMs = DateTime.now().millisecondsSinceEpoch;
    final currentExpiresAt = prefs.getInt('pending_momo_expires_at_ms') ?? (nowMs + (_remainingSeconds * 1000));
    final newExpiresAt = currentExpiresAt + 30000;
    await prefs.setInt('pending_momo_expires_at_ms', newExpiresAt);

    final remainingMs = newExpiresAt - nowMs;
    _startPendingTimer((remainingMs / 1000).ceil());
  }

  Future<void> clearPendingVerification() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('pending_momo_txn_id');
    await prefs.remove('pending_momo_created_at_ms');
    await prefs.remove('pending_momo_expires_at_ms');

    _hasPendingVerify = false;
    _pendingTxnId = null;
    _remainingSeconds = 0;
    _pendingTimer?.cancel();
    _pendingTimer = null;
  ...
  Future<void> _savePendingVerification(String transactionId) async {
    final prefs = await SharedPreferences.getInstance();
    final nowMs = DateTime.now().millisecondsSinceEpoch;
    final expiresAtMs = nowMs + 60000; // 60 seconds
    await prefs.setString('pending_momo_txn_id', transactionId);
    await prefs.setInt('pending_momo_created_at_ms', nowMs);
    await prefs.setInt('pending_momo_expires_at_ms', expiresAtMs);

    _hasPendingVerify = true;
    _pendingTxnId = transactionId;
    _startPendingTimer(60);
  }

  Future<void> loadPendingVerification() async {
    final prefs = await SharedPreferences.getInstance();
    final txnId = prefs.getString('pending_momo_txn_id');
    if (txnId == null || txnId.isEmpty) return;

    final nowMs = DateTime.now().millisecondsSinceEpoch;
    var expiresAtMs = prefs.getInt('pending_momo_expires_at_ms');
    if (expiresAtMs == null) {
      clearPendingVerification();
      return;
    }
  ...
  Future<bool> initiateMomoPayment(
      String phone,
      String amount,
      ...
      List<XFile>? orderAttachment) async {
    if (phone.isEmpty) {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://onestop.visionvivante.in/api/v1/auth/momo_payment'),
        headers: {
          'server-key': '2cfef782dd7059f7c151d409274851f881321407',
          'access-token': Get.find<AuthController>().getUserToken(),
          'Content-Type': 'application/json',
        },
        body: jsonEncode({
          "amount": double.tryParse(amount)?.toInt() ?? 0,
          "phone_number": int.tryParse(phone) ?? 0,
          "payment_method": "momo",
          'description': 'One Stop Order Payment',
        }),
      );
      ...
      if (data['status'] == 'success') {
        ...
        _pendingMaximumCodOrderAmount = maximumCodOrderAmount;
        _pendingFromCart = fromCart;
        _pendingIsCashOnDeliveryActive = isCashOnDeliveryActive;
        _pendingOrderAttachment = orderAttachment;

        await _savePendingVerification(data['transaction_id']);
        _isLoading = false;
        update();
        return true;
      } else {
        showCustomSnackBar(data['message'] ?? 'Payment failed');
      }
    } catch (e) {
  ...
  Future<void> verifyMomoPayment() async {
    if (_pendingTxnId == null || _pendingTxnId!.isEmpty) {
      showCustomSnackBar("Transaction ID missing for verification");
      return;
    }
    _isLoading = true;
    update();
    try {
      final response = await http.post(
        Uri.parse('https://plusliberia.visionvivante.in/endpoint/auth/verify_momo_payment'),
        headers: {
          'server-key': '4b22195db4325b83091c0f422d17991dd78dc0a4',
          'access-token': Get.find<AuthController>().getUserToken(),
          'Content-Type': 'application/json',
        },
        body: jsonEncode({"reference_id": _pendingTxnId}),
      );

      final rawBody = response.body.trim();
      var jsonBody = rawBody.replaceAll(RegExp(r'<\/?pre>'), '').trim();
      final start = jsonBody.indexOf('{');
      final end = jsonBody.lastIndexOf('}');
      if (start != -1 && end != -1 && end > start) {
        jsonBody = jsonBody.substring(start, end + 1);
      }
      final data = jsonDecode(jsonBody);
      final status = (data['status'] ?? '').toString().toUpperCase();
      
      if (status == 'SUCCESSFUL') {
        if (_pendingPlaceOrderBody != null) {
          // Pass the transaction ID to backend (append to order note as workaround)
          _pendingPlaceOrderBody = PlaceOrderBodyModel(
            cart: _pendingPlaceOrderBody!.cart!,
            couponDiscountAmount: _pendingPlaceOrderBody!.couponDiscountAmount,
            couponCode: _pendingPlaceOrderBody!.couponCode,
            orderAmount: _pendingPlaceOrderBody!.orderAmount ?? 0,
            orderType: _pendingPlaceOrderBody!.orderType,
            paymentMethod: 'digital_payment', // Backend might recognize this
            storeId: _pendingPlaceOrderBody!.storeId,
            distance: _pendingPlaceOrderBody!.distance,
            scheduleAt: _pendingPlaceOrderBody!.scheduleAt,
            discountAmount: _pendingPlaceOrderBody!.discountAmount,
            taxAmount: _pendingPlaceOrderBody!.taxAmount,
            orderNote: '${_pendingPlaceOrderBody!.orderNote ?? ''} [MoMo Txn: $_pendingTxnId]',
            address: _pendingPlaceOrderBody!.address,
            receiverDetails: _pendingPlaceOrderBody!.receiverDetails,
            latitude: _pendingPlaceOrderBody!.latitude,
            longitude: _pendingPlaceOrderBody!.longitude,
            senderZoneId: _pendingPlaceOrderBody!.senderZoneId,
            contactPersonName: _pendingPlaceOrderBody!.contactPersonName,
            contactPersonNumber: _pendingPlaceOrderBody!.contactPersonNumber,
            addressType: _pendingPlaceOrderBody!.addressType,
            parcelCategoryId: _pendingPlaceOrderBody!.parcelCategoryId,
            chargePayer: _pendingPlaceOrderBody!.chargePayer,
            streetNumber: _pendingPlaceOrderBody!.streetNumber,
            house: _pendingPlaceOrderBody!.house,
            floor: _pendingPlaceOrderBody!.floor,
            dmTips: _pendingPlaceOrderBody!.dmTips ?? '',
            unavailableItemNote: _pendingPlaceOrderBody!.unavailableItemNote ?? '',
            deliveryInstruction: _pendingPlaceOrderBody!.deliveryInstruction,
            cutlery: _pendingPlaceOrderBody!.cutlery ?? 0,
            partialPayment: _pendingPlaceOrderBody!.partialPayment ?? 0,
            guestId: _pendingPlaceOrderBody!.guestId ?? 0,
            isBuyNow: _pendingPlaceOrderBody!.isBuyNow ?? 0,
            guestEmail: _pendingPlaceOrderBody!.guestEmail,
            extraPackagingAmount: _pendingPlaceOrderBody!.extraPackagingAmount,
            createNewUser: _pendingPlaceOrderBody!.createNewUser,
            password: _pendingPlaceOrderBody!.password,
            isPrescriptionOrder: _pendingPlaceOrderBody!.isPrescriptionOrder,
            bringChangeAmount: _pendingPlaceOrderBody!.bringChangeAmount,
          );
          
          await placeOrder(
            _pendingPlaceOrderBody!, 
            _pendingZoneID, 
            _pendingAmount, 
            _pendingMaximumCodOrderAmount, 
            _pendingFromCart, 
            _pendingIsCashOnDeliveryActive, 
            _pendingOrderAttachment
          );
        }
        await clearPendingVerification();
      } else if (status == 'FAILED') {
        await clearPendingVerification();
        showCustomSnackBar("Payment failed or invalid mobile number.");
      } else {
        showCustomSnackBar(data['message'] ?? 'Payment not verified yet. Please try again.');
      }
    } catch (e) {
      showCustomSnackBar(e.toString());
    }
